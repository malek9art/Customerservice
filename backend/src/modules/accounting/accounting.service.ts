...
export class AccountingService {
  ...
  @OnEvent('flight.booking_created') handleFlight(payload: any) {
    return this.createServiceInvoice(payload, 'FLIGHT', 'Flight booking');
  }
  @OnEvent('hotel.booking_created') handleHotel(payload: any) {
    return this.createServiceInvoice(payload, 'HOTEL', 'Hotel booking');
  }
  @OnEvent('pilgrimage.booking_created') handlePilgrimage(payload: any) {
    return this.createServiceInvoice(
      payload,
      'PILGRIMAGE',
      'Hajj/Umrah booking',
    );
  }
  @OnEvent('visa.application_created') handleVisa(payload: any) {
    return this.createServiceInvoice(payload, 'VISA', 'Visa application');
  }
  ...
  async createInvoice(
    ...
    if (data.sourceId) {
      const existing = await (this.prisma as any).invoice.findFirst({
        where: { companyId, sourceId: data.sourceId },
      });
      if (existing) return this.enrichInvoice(existing);
    }
    const items: InvoiceItemDto[] = data.items?.length
      ? data.items
      : data.amount && data.amount > 0
        ? [{ description: 'Invoice', quantity: 1, unitPrice: data.amount }]
        : [];
    if (!items.length)
    ...
    if (discount > subtotal)
      throw new BadRequestException('Discount cannot exceed invoice subtotal');
    ...
    const invoice = await (this.prisma as any).invoice.create({
      data: {
        ...
        number: data.number || `INV-${nanoid(10).toUpperCase()}`,
        subtotal,
        discount,
    ...
    await this.postingEngine.postEvent(
      companyId,
      this.eventType(data.sourceType),
      {
        id: invoice.id,
        customerId: data.customerId,
        amount: total,
    ...
    await this.activity(
      data.customerId,
      'INVOICE_CREATED',
      `Invoice ${invoice.number} created for ${total} ${invoice.currency}`,
    );
  ...
  async recordPayment(
    ...
    const balance = Number(
      invoice.balance ?? invoice.amount - (invoice.paidAmount || 0),
    );
    ...
    await this.activity(
      invoice.customerId,
      'PAYMENT_RECEIVED',
      `${data.method} payment ${data.amount} received for ${invoice.number}`,
    );
  ...
  async listCustomerInvoices(companyId: string, customerId: string) {
    ...
    return Promise.all(
      invoices.map((invoice: any) => this.enrichInvoice(invoice)),
    );
  }
  ...
  async customerStatement(companyId: string, customerId: string) {
    ...
    return {
      ...
      invoices,
      payments,
      total: invoices.reduce((s: number, i: any) => s + Number(i.amount), 0),
  ...
  async ledger(companyId: string) {
    ...
    return Promise.all(
      journals.map(async (journal: any) => ({
        ...journal,
        entries: await (this.prisma as any).journalEntry.findMany({
  ...
  async getDashboard(companyId: string) {
    ...
    return {
      ...
      invoicesToday: invoices.filter(
        (i: any) => new Date(i.createdAt).toISOString().slice(0, 10) === today,
      ).length,
      paymentsToday: payments.filter(
        (p: any) => new Date(p.createdAt).toISOString().slice(0, 10) === today,
      ).length,
      ...
      items: [
        {
          ...
          serviceId: payload.bookingId || payload.id,
        },
      ],
      taxRate: 0,
      discount: 0,
  ...
  private async enrichInvoice(invoice: any) {
    const payments = await (this.prisma as any).payment.findMany({
      where: { invoiceId: invoice.id },
    });
    return { ...invoice, payments };
  }
  private async ownedInvoice(companyId: string, id: string) {
  ...
  private async assertCustomer(companyId: string, id: string) {
    const c = await (this.prisma as any).customer.findUnique({ where: { id } });
    if (!c || c.companyId !== companyId)
      throw new NotFoundException('Customer not found');
  }
  private activity(customerId: string, action: string, description: string) {
