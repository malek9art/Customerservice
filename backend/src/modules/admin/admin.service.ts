...
export class AdminService {
  ...
  listUsers(companyId: string) {
    return (this.prisma as any).employee.findMany({ where: { companyId } });
  }
  ...
  async getCommandCenter(companyId: string) {
    const [users, customers, activities] = await Promise.all([
      this.listUsers(companyId),
      (this.prisma as any).customer.findMany({ where: { companyId } }),
      this.getSystemLogs(companyId),
    ]);
    ...
    return {
      stats: {
        ...
        memoryRecords: Object.values(memory).reduce((s, n) => s + Number(n), 0),
  ...
  async getSystemLogs(companyId: string) {
    ...
    const ids = new Set(customers.map((c: any) => c.id));
    const logs = await (this.prisma as any).activityLog.findMany({
      orderBy: { createdAt: 'desc' },
    });
    return logs.filter((l: any) => ids.has(l.customerId)).slice(0, 100);
  }
  ...
  async updateSettings(companyId: string, settings: Record<string, unknown>) {
    ...
    return (this.prisma as any).company.update({
      where: { id: company.id },
      data: { settings: { ...(company.settings || {}), ...settings } },
    });
  }
  private async company(id: string) {
