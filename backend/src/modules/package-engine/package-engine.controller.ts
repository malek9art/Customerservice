...
export class PackageEngineController {
  ...
  list(@CurrentCompany() companyId: string) {
    return this.packageEngine.listPackages(companyId);
  }

  @Post()
  @ApiOperation({ summary: 'Create a travel package' })
  @ApiHeader({ name: 'x-company-id', required: true })
  create(@CurrentCompany() companyId: string, @Body() data: CreatePackageDto) {
  ...
  @ApiHeader({ name: 'x-company-id', required: true })
  getOne(@CurrentCompany() companyId: string, @Param('id') id: string) {
    return this.packageEngine.getPackageDetails(companyId, id);
  }
  ...
  update(
    ...
    return this.packageEngine.updatePackage(companyId, id, data);
  }
  ...
  capacity(
    @CurrentCompany() companyId: string,
    @Param('id') id: string,
    @Body() data: UpdatePackageCapacityDto,
  ) {
    return this.packageEngine.setCapacity(companyId, id, data.capacity);
  }
}
